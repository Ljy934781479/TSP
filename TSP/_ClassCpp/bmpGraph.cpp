...
vector<short> CBMPgraph::dpAl1g()
{
	...
	UINT route = 0;//利用二进制为回溯哪个点已经用过了
	UINT maxInt = pow(2, sizeof(UINT) * 8) - 1;//这个用来回溯路径
	maxInt = 1000;
	vector<short> vPath;//保存路径顺序
	UINT setId = 0;//这个用来回溯路径
	map<UINT, UINT> mTonode;//集合点数量,所走的点
	map<UINT, UINT> mDistance;//集合点数量,最小距离
	UINT pathNode;
	int n = _vGraph.size();
	n = 4;
	if (n > sizeof(UINT) * 8)
	...
	{//数字0 - [m-1] 的二进制数,可以一对一的表示出各个子集顶点的存在情况，但是需要注意并不是那么的有序
		...
		{//如果这个集合的数字的二进制i位为1则不行,i到达不了这个集合
			...
			dp[i][j] = minCost;
			if (i != 0 && minCost < mDistance[cntNodes])
			{
				mDistance[cntNodes] = minCost;
				mTonode[cntNodes] = i;
			}
		}
	}
	...
	while (setId)
	{
		UINT minCost = 99999;//维护最小花费
		vector<UINT> vSet2;//次级子集
		for (UINT j = 1; j < m; j++)
		{//满足次级子集二进制1少一个
			int cnt1 = 0;
			int cnt2 = 0;
			int offset = 0;
			bool subSet = true;
			while (offset != sizeof(UINT) * 8)
			{
				UINT j1 = j >> offset & 1;
				UINT setId2 = setId >> offset & 1;
				if (j1 == 1) cnt2++;
				if (setId2 == 1) cnt1++;
				//如果j含有setId没有的点
				if (j1 == 1 && setId2 == 0)
		...
		if (vSet2.size() == 0)
		{
			int temp = getLowZero(route);
			vPath.push_back(temp);//更新路径
			mTonode[0] = temp;
			break;
		}
		UINT head = pathNode;
		for (UINT i = 1; i < n; i++)//遍历所有能到达这些子集的点,更新最小花费和最新的点位
		{//并且不能是已经走过的点
			for (UINT& j : vSet2)
			{//检查i点是否能经过这个集合,如果这个集合的数字的二进制i位为1则不行
				if (((j >> (i - 1)) & 1) == 1)
					continue;
				if (((route >> (i - 1)) & 1) == 1)//并且不能是已经走过的点
					continue;
				if (dp[i][j] + getDis(head,i) >= minCost)
					continue;
				minCost = dp[i][j] + getDis(head,i);
				pathNode = i;
				setId = j;//更新下一个集合
			}
		}
		route = route | (1 << (pathNode - 1));
		vPath.push_back(pathNode);//更新路径
	}
	return vPath;
}
